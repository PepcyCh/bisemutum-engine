...
template <>
struct std::hash<bismuth::gfx::BufferKey> {
    size_t operator()(const bismuth::gfx::BufferKey &v) const noexcept {
        return bismuth::Hash(v.size, v.usages, v.memory_property);
    }
};

template <>
struct std::hash<bismuth::gfx::TextureKey> {
    size_t operator()(const bismuth::gfx::TextureKey &v) const noexcept {
