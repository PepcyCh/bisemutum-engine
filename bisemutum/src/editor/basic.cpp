...
auto edit_int(std::string_view name, int& value, float speed, int min, int max) -> bool {
    return ImGui::DragInt(name.data(), &value, speed, max, max);
}
auto edit_int2(std::string_view name, int2& value, float speed, int min, int max) -> bool {
    return ImGui::DragInt2(name.data(), &value.x, speed, max, max);
}
auto edit_int3(std::string_view name, int3& value, float speed, int min, int max) -> bool {
    return ImGui::DragInt3(name.data(), &value.x, speed, max, max);
}
auto edit_int4(std::string_view name, int4& value, float speed, int min, int max) -> bool {
    return ImGui::DragInt4(name.data(), &value.x, speed, max, max);
}

auto edit_uint(std::string_view name, uint32_t& value, float speed, uint32_t min, uint32_t max) -> bool {
    auto int_value = static_cast<int>(value);
    auto dirty = ImGui::DragInt(name.data(), &int_value, speed, min, max);
    value = int_value;
    return dirty;
}
auto edit_uint2(std::string_view name, uint2& value, float speed, uint32_t min, uint32_t max) -> bool {
    auto int_value = int2{value};
    auto dirty = ImGui::DragInt2(name.data(), &int_value.x, speed, min, max);
    value = int_value;
    return dirty;
}
auto edit_uint3(std::string_view name, uint3& value, float speed, uint32_t min, uint32_t max) -> bool {
    auto int_value = int3{value};
    auto dirty = ImGui::DragInt3(name.data(), &int_value.x, speed, min, max);
    value = int_value;
    return dirty;
}
auto edit_uint4(std::string_view name, uint4& value, float speed, uint32_t min, uint32_t max) -> bool {
